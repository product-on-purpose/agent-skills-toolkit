...
- **A command over 4000 bytes is SILENTLY DROPPED by the command migration, not truncated. Read 2026-09-05 from SOURCE CODE, `method: read`.** `openai/codex` at commit `52e73e3a548ae5310c7765995b9803dd538b82b0`: `codex-rs/core-plugins/src/command_migration/plugin.rs` line 17 sets `const MAX_MIGRATED_COMMAND_SKILL_BYTES: usize = 4_000;`, and `codex-rs/core-plugins/src/command_migration.rs` lines 165-169 enforce it with `continue` - the loop moves to the next command and no `SKILL.md` is written at all. **This is a SOURCE READ and deliberately NOT a ledger claim.** The fact lives in a Rust source file rather than on a documentation page, so the `quote` mechanism has nothing to re-fetch and pinning it would mint an age-blocking claim no instrument can re-verify. It therefore does not block a release, and it also does not re-verify itself: **the next Codex survey MUST re-read both files at a fresh commit and update the constant in `scripts/checks/command-size-cap.mjs` if it moved.** The cap is enforced against the RENDERED skill (`render_command_skill()` replaces the command's frontmatter with generated skill frontmatter), which is why `U18` measures the source file as a declared proxy at `warn` rather than asserting an exact rendered size. Landed as `U18` (`command-size-cap`) with ADR 0058. **Correction recorded:** the resolution plan's RS-C3 row called this "silent truncation"; the source says skip.
