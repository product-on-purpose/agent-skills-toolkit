...
test("the spine is exactly 33 checks (U1-U9, U11-U16 + S1-S8 + G1-G10)", () => {
...
// discrepancies.
test("the reqId -> since map matches the ADR 0027 + ADR 0035 + ADR 0045 + ADR 0046 + ADR 0050 baseline table [R-SINCE-2]", () => {
  const since010 = new Set(["U12", "G7", "G8", "G9", "G10"]);
  const since012 = new Set(["U13"]);
  const since013 = new Set(["U14"]);
  const since014 = new Set(["U15", "U16"]);
  for (const m of CHECKS) {
