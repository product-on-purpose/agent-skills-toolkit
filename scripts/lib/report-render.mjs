...
function deriveModel(report, opts = {}) {
  ...
  return {
    ...
    behavioral: report.behavioral ?? null,
...
function renderMarkdown(report, opts = {}) {
  ...
  if (m.reportType === "review" && m.review) {
    ...
    out.push("");
    out.push(`**Advisory layer, generated by ${rev.model} (effort ${rev.effort}, ${rev.date}). This is judgment over sampled inputs, not a gate result: it never moves the grade or the gate exit code.**`);
    out.push("");
    ...
    out.push("");
    out.push(`**Advisory layer, generated by ${bh.model} (effort ${bh.effort}, ${bh.date}). This is evidence over sampled cases, not a gate result: it never fails CI or moves the grade.**`);
    out.push("");
    out.push(`Summary: ${bh.summary.fired} fired, ${bh.summary.missed} missed, ${bh.summary.behaviorPass} behavior pass, ${bh.summary.behaviorFail} behavior fail.`);
    out.push("");
  ...
  if (advStampMd) {
    out.push(`Advisory provenance: this report carries an advisory ${m.reportType} layer generated by ${advStampMd.model} (effort ${advStampMd.effort}, ${advStampMd.date}). The advisory layer is judgment over sampled inputs: it is clearly labeled, and it never changes the deterministic grade or the gate exit code.`);
    out.push("");
  }
