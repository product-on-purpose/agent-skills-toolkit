...
- `action-pin-watch.mjs` - the action-pin watch (E45, ADR 0053): resolves every `uses:` pin across the workflows and `action.yml` against the GitHub registry and reports where a pin s human-readable LABEL disagrees with what its machine-readable REF resolves to. Write-incapable. Exits 1 on a label problem and 2 when a lookup could not be performed; a pin merely BEHIND its action s current release is reported and exits 0, because that is somebody else s release cadence rather than a defect here.
