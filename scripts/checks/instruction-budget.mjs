...
// scope-note:  skills only, BY DECISION (ADR 0048, as amended 2026-08-15). A command IS a skill - Claude Code
//               merged commands into skills - but it is not held to the skill instruction budget, because the
//               budget exists so later steps are not silently dropped from a body a MODEL loads, and a command
//               is user-invoked. Measured across the reference family, commands run a median 172 words against a
