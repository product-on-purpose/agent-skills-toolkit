...
export function check(ctx) {
  ...
  if (!existsSync(p)) {
    return [finding(meta.id, SEVERITY.ERROR, "INDEX.md is missing; at Gold it is generated by gen-index and drift-checked (Standard sec 2.6 G4). Generate it: npx agent-skills-toolkit gen-index . --write (or `node scripts/generators/gen-index.mjs . --write` if you vendor this toolkit)", { file: "INDEX.md", reqId: meta.reqId })];
  }
  let onDisk;
  try {
  ...
  if (norm(onDisk) !== norm(renderIndex(ctx))) {
    ...
    if (norm(onDisk) === norm(renderLegacyIndex(ctx))) {
      // The remediation names ONE command, and it is a subcommand of the package this project actually
      // owns. The first version of this message offered a hyphenated `npx <toolkit>-gen-index` name that
      // this repository does not publish - unusable as an instruction, and a package-claim supply-chain
      // risk. It was also E35 itself one level down: a remediation naming a command its reader does not
      // have, inside the finding written to fix exactly that. The exact string is deliberately not
      // repeated here, because tests/unit/trust-visibility.test.mjs greps this file for it.
      return [finding(meta.id, SEVERITY.ERROR,
        "INDEX.md matches the pre-v1.13.0 rendering exactly: this toolkit's own generator emitted a `Self-validating: node scripts/check.mjs` line into every plugin, including plugins that do not vendor the gate (E35). Regenerate to pick up the corrected line: npx agent-skills-toolkit gen-index . --write. If your plugin DOES vendor this toolkit's gate, declare `\"selfValidation\": \"vendored\"` in library.json first, then regenerate.",
        { file: "INDEX.md", reqId: meta.reqId, migration: LEGACY_INDEX_MIGRATION })];
    }
    return [finding(meta.id, SEVERITY.ERROR, "INDEX.md is out of date with library.json + component frontmatter (a hand-edited generated file is an error at Gold, Standard sec 2.6 G4). Regenerate: npx agent-skills-toolkit gen-index . --write (or `node scripts/generators/gen-index.mjs . --write` if you vendor this toolkit)", { file: "INDEX.md", reqId: meta.reqId })];
  }
  return [];
}
