...
- **Model tier changes WHAT KIND of finding is reachable, not just depth (the first same-target A/B).** On an identical small plugin, Haiku/medium (~55k) reported "no veracity gaps detected" while Opus/high (~100k, ~1.8x) hand-verified the content and found a wrong statute name in a legal-compliance skill, an inverted grammar rule taught as fact, and a command whose scorecard contradicts the skill it claims to apply. Both agreed on the structural read. If the review must vouch for factual, legal, or domain claims, budget Opus/high; a cheaper tier's "verified" is not verification.
