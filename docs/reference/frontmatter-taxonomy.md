...
> **Status.** The taxonomy is applied and **enforced** as of Standard v0.10: the `docs-frontmatter` check is `G7` (the spine is 33 checks, `U1-U9` + `U11-U16` + `S1-S8` + `G1-G10`). The related `docs-presence` (`G10`), folder-README (`G8`), and source-docblock (`G9`) checks are all live. See [How it is enforced](#how-it-is-enforced).
