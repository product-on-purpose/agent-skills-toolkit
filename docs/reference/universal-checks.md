...
|---|---|---|---|---|---|
| U1 | `scripts/checks/library-json.mjs` | `library.json` is present and valid JSON with the identity fields a tool needs (`name`, `version`, `tier`, `standard`) | sec 5, sec 5.1 | no | Add or repair `library.json` (use `askit-build-settings`), then re-run `npx agent-skills-toolkit`. |
