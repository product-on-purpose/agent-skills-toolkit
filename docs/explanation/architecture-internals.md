...
A conformance check is a small ES module under `scripts/checks/` with exactly two exports: a `meta` object and a synchronous `check(ctx)` function. The contract is uniform across all 32 spine checks. Here is `scripts/checks/library-json.mjs` (the `U1` manifest check), trimmed to its shape:
...
export const CHECKS = [
  ...
  // ... abridged; see scripts/lib/registry.mjs for the full ordered 32-check list
];
