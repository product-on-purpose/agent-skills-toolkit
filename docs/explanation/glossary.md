...
**Spine** - the 30-check backbone the toolkit ships (`U1`-`U9`, `U11`-`U13`, `S1`-`S8`, `G1`-`G10`); the `G7` slot is the `docs-frontmatter` check (Standard v0.10), `U13` (`skill-registration`) was added in Standard v0.12, and tier inclusion of Bronze and Silver is a structural property of the monotonic tiers, not a numbered check.
