...
The spine is **25 checks** total - `U1-U11` (Universal), `S1-S8` (Convergent), and `G1-G6` (Advanced); the Gold requirement `G7` is tier inclusion, satisfied structurally rather than by a separate check. Both a machine-readable form (`node scripts/tier-report.mjs --json`) and a one-line human summary come out of the same computation, so tooling and a person read the same verdict.
