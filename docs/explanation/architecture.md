...
The spine is **34 checks** total - `U1-U9`, `U11-U17` (Universal), `S1-S8` (Convergent), and `G1-G10` (Advanced); the `G7` slot is the `docs-frontmatter` check (Standard v0.10), `U13` (`skill-registration`) was added in Standard v0.12, `U14` (`agent-restricted-fields`) in Standard v0.13, and tier inclusion of Bronze and Silver is a structural property of the monotonic tiers, not a numbered check. Both a machine-readable form (`node scripts/tier-report.mjs --json`) and a one-line human summary come out of the same computation, so tooling and a person read the same verdict.
