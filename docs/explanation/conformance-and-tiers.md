...
`tier-report` caps `satisfies` at the plugin's declared tier (so a Bronze plugin cannot accidentally claim Silver), and lists everything above the ceiling as `blocked.<next-tier>`. The gate-runner (`check.mjs`) follows the same model: only errors at-or-below the declared tier fail the gate. So a Bronze plugin that adds Silver requirements gradually sees its `blocked.convergent` list shrink, while CI stays green throughout the climb. The repository completed the full climb to Gold: the toolkit now declares `tier: advanced` and `blocked` is empty (Bronze, Silver, and Gold all satisfied). The Silver checks (S1-S8) and Gold checks (G1-G10) are documented in [`../reference/silver-checks.md`](../reference/silver-checks.md) and [`../reference/gold-checks.md`](../reference/gold-checks.md), and the Universal floor in [`../reference/universal-checks.md`](../reference/universal-checks.md).
