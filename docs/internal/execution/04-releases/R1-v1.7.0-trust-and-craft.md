...
### H1.9 - remove the orphaned catalog build artifact - REJECTED 2026-07-26, the premise is false

**Original item (do not execute):** "`site/src/content/docs/catalog.md` is a gitignored build artifact left on disk. Remove it (site-content cleanup); confirm the site build regenerates cleanly without it."

**Why it was rejected.** Every clause of that premise is wrong, verified against the live tree:

| Claim | Reality |
|---|---|
| "gitignored" | Matched by no `.gitignore` rule (`git check-ignore` returns nothing) |
| "build artifact" | `site/scripts/gen-docs-site.mjs` contains no reference to `catalog`; the generator emits into the quadrant SUBDIRECTORIES, and files directly under `site/src/content/docs/` are the hand-authored landing pages |
| "left on disk" | Tracked in git since `f5291c0` (the v1.0.0 marketplace launch) and maintained across four commits since |
| "no route regression" | `/catalog/index.html` is in `site/scripts/route-manifest.txt`, and `site/astro.config.mjs` gives it a named sidebar section ("The catalog") |

It is also guarded by `tests/unit/catalog-coverage.test.mjs`, whose own header says: *"hand-authored editorial prose, NOT generated: it carries annotations that do not exist in library.json, so a generator would flatten the voice. This test is its drift guard instead."*

Executing the item as written deleted a published public documentation page, its route, its sidebar entry, and its drift-guard test, taking the site from 77 pages to 75. All five deletions were reverted.

**The lesson, recorded because it generalizes.** A confidently-worded plan makes this failure mode MORE likely, not less: the premise gets executed without re-derivation against the tree. This is precisely the stop-and-flag condition [03-execution-plan.md](../03-execution-plan.md) section 5 already names ("a plan assumption fails against the live tree: re-derive; if the shape of the plan changes, stop and flag"). It fired as designed.

- **Disposition:** no action. If a genuinely orphaned site artifact is found later, it needs its own item with the evidence attached, not this one.
