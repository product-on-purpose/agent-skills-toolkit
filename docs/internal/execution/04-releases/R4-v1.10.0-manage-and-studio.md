...
Engineering summary: R4 adds one guided removal-execution path to `askit-deprecate`, one ADR-gated validator for workflow step orphans (spine check or non-spine validator, decided ADR-first per the ADR 0035 (U13 skill-registration) precedent), and a `127.0.0.1`-bound, token-gated, zero-runtime-dependency Node server that runs the existing `tier-report` / `evaluate` JSON against a chosen target and renders it as a dashboard page. The optional riders serialize data the gate already computes; both SHIPPED in v1.11.0 on 2026-08-11. Everything is built cleanly separable from the checker per Ruling 4.
...
## 1. Scope, the two workstreams, and the optional riders
...
| E4 (SARIF output) - optional rider, SHIPPED v1.11.0 | SARIF 2.1.0 + GitHub Actions annotations as output formats | Sonnet | Serializer |
| E9 (provenance output contract) - optional rider, SHIPPED v1.11.0 | Stable machine-readable provenance tag on every finding | Sonnet | Schema + docs |

The two optional riders are independently droppable. If R4 fills up on SP4 plus the Studio slice, E4 (SARIF output) and E9 (provenance output contract) fall back to the backlog untouched; neither is a release gate. **Outcome: both SHIPPED in v1.11.0, 2026-08-11.**
...
| R4-7 | E9 provenance contract (optional rider; SHIPPED v1.11.0) | none | Sonnet |
| R4-8 | E4 SARIF + annotations (optional rider; SHIPPED v1.11.0) | R4-7 if both ride | Sonnet |
...
flowchart TD
    ...
    F --> Z
    G["R4-7 E9 provenance rider"] --> H["R4-8 E4 SARIF rider"]
    G --> Z
    H --> Z
```
...
| R4-RK-6 | Program runs long; optional riders half-built at the close | Medium | Low | E4/E9 are gated all-or-nothing in the exit gate; a half-built rider is deferred whole, never merged partial. **Did not occur: both SHIPPED in v1.11.0, 2026-08-11** |
