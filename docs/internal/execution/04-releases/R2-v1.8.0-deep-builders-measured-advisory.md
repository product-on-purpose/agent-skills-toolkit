...
- [x] Evals: real `covers.skill` triggering sets for 3 shipped skills land. **Criterion corrected 2026-07-26, twice.** (a) The premise "the convention has zero on-disk instances" was stale: seven instances already existed, covering all five chain edges and one hook. The real gap was `covers.skill`, of which there were zero, and that is what shipped. (b) "**and G3 grades them**" is **unsatisfiable as written**. `G3` (`library-regression`) accepts a `covers.skill` set but does not grade it: its skill branch sets `claimed = true` and does nothing else, with the in-code comment "a triggering eval set (a Universal SHOULD per sec 8.3); not gated by the G3 baseline". `G3` requires an eval per chain edge and per hook event only. Nothing in the spine validates a triggering set's shape, case count, or `should_trigger` field. The three sets were therefore hand-verified instead: 24 cases each (Standard sec 8.3 asks for at least 20), zero malformed entries, correct `covers`. **Filed as a candidate for a later release:** shape validation for `covers.skill` sets. Deliberately NOT done here, because R2's own non-goals forbid a check or severity change, and this would turn currently-passing files into potentially-failing ones.
