...
- [ ] **npm publish**, for any release that changes shipped code. The gate is a published package (`agent-skills-toolkit` on npm) as of v1.11.0, and until v1.12.0 this checklist did not mention it at all - so a release could be tagged, GitHub-released and marketplace re-pinned while npm consumers silently stayed on the previous version. Publish via the `workflow_dispatch`-gated `publish-npm.yml` (which re-runs the suite and the gate and enforces tag-versus-manifest agreement), or locally with `npm publish` if the workflow is unavailable. **Two constraints that are not negotiable and are recorded here rather than rediscovered:** the account has 2FA (`auth-and-writes`) enforced, so a local publish needs a human at the prompt and cannot be automated; and `--provenance` cannot be produced from a laptop, so a local publish either accepts no provenance or waits for the workflow. Verify from the consumer position afterwards - install from the live registry into a clean directory outside this repository and grade a plugin with `npx` - because a published manifest is a claim until someone installs it.
