...
- **Do NOT "fix" it by widening the threshold.** The 5000 ms bar is not arbitrary padding - it IS the helper's lifetime, and that is the only thing distinguishing "the process did not wait" from "the process waited and the helper happened to die". Raising the bar above the helper's lifetime removes the test's entire discriminating power while making it green, which is worse than a flake.
- **The right fix is to widen the GAP, not the bar:** give the stuck helper a much longer lifetime (30 s or more) so that an exit at 5 s is unambiguous under any plausible scheduling delay, and keep asserting against the helper's lifetime rather than a fixed number.
...
- **Status:** backlog (recorded 2026-08-12, during the v1.12.0 round-2 fixes).
...
- **Status:** backlog (recorded 2026-08-12, from the v1.12.0 adversarial review).
...
- **The fix is one line** - make the bit conditional on `existsSync(<root>/scripts/check.mjs)`, and emit `npx agent-skills-toolkit .` otherwise. It was written, tested and **reverted inside the v1.12.0 cut**.
...
- **Status:** backlog (recorded 2026-08-12, during the v1.12.0 cut, after being implemented and reverted in the same cut).

### E34 - graduate the marketplace-scope findings to numbered spine checks  [design, effort M, ADR-gated, scheduled]
...
- **Status:** backlog (recorded 2026-08-12, during the v1.12.0 cut).
...
- **Status:** backlog (recorded 2026-08-11, during the v1.11.1 cut).
