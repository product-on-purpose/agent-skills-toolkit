...
- **The rule that resolves all three:** *a control that lives inside the artifact it controls is not a control.* In this repository the only trust roots that hold are **repository settings**: branch protection, and environment deployment branch policies. Both surviving controls (`github-pages`, `npm-publish`) are environment policies for exactly this reason.
- **Why it is worth a check rather than a note:** the rule was learned three times in one release by an author who had written it down after the first time. Prose did not prevent the second or third instance. That is the same evidence pattern that justified `scripts/check-release-counts.mjs` in v1.10.1, where a hand-maintained number went stale four times while everyone involved was actively looking for it.
- **Candidate change:** a test asserting that any workflow granting `id-token: write`, holding a publish or deploy step, or referencing a secret, declares an `environment:`; plus a companion record of which environments must carry a branch policy, verified against the live API during the release gate rather than assumed. The hard part is the second half, since repository settings are not in the tree; decide whether the release gate queries them or whether a documented manual verification is honest enough.
- **Watch out for:** this must not become a check that asserts a setting exists without confirming its value. An environment with no branch policy satisfies a naive "declares an environment" test while providing none of the protection, which would be a new instance of the very defect class it is meant to close.
