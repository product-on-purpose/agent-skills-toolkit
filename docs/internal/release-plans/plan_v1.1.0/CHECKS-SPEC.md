...
2. The first non-blank line begins with a recognized diagram keyword. The set MUST cover the current Mermaid registry (one named constant, easy to extend): the classic types (`flowchart`, `graph`, `sequenceDiagram`, `classDiagram`, `stateDiagram`(`-v2`), `erDiagram`, `journey`, `gantt`, `pie`, `mindmap`, `timeline`, `quadrantChart`, `gitGraph`), the **full C4 family** (`C4Context`, `C4Container`, `C4Component`, `C4Dynamic`, `C4Deployment`), and the newer types (`xychart-beta`, `sankey-beta`, `block-beta`/`block`, `requirementDiagram`, `packet-beta`/`packet`, `kanban`, `architecture-beta`, `radar-beta`, `treemap`, `zenuml`, `info`). Omitting a registered type is a Bronze false-fail that would block any downstream plugin shipping it.
...
**Scope:** all tracked `*.md` and `*.mdx` files, via its own file walk. Note `U10` (`no-dashes`) scans only `.md`/`.mjs` (its `SCAN` regex has no `.mdx` branch), so `U12` cannot literally reuse that scanner - it adds `.mdx` (the site landing `site/src/content/docs/index.mdx` carries diagrams). Reuse the U10 directory skip-set **by basename** (`node_modules` at any depth, `.git`, `.memsearch`, `_LOCAL`/`_local`, `_agent-context`, `dist`, `.astro`) by importing the shared `SKIP_DIRS` constant rather than re-listing it (there is no literal `site/node_modules` entry; nested `node_modules` is matched by basename). **Also skip the generated Pattern S quadrant subtrees** (the immediate subdirectories of `site/src/content/docs/`): they are emitted from `docs/**` on every build and gitignored, so scanning them would make U12's result depend on build state; their blocks are verbatim copies already validated at their `docs/` source. The hand-authored landing pages (files directly under `site/src/content/docs/`) stay in scope. The fence opener allows leading indentation and an info string after `mermaid` so an indented MDX fence is not silently skipped.
