...
The flagged decisions, resolved while building P4 and reflected in the shipped `folder-readme.mjs`:

1. **Repo root `.` is EXCLUDED from the allowlist.** Folder-guide frontmatter (`title:`) on the root `README.md` degrades the project's GitHub-rendered hero (GitHub renders YAML frontmatter as a table above the centered hero), and root navigation is already served by the hero's "Repository map" section, `AGENTS.md`, and `INDEX.md`. The D1.1 convention targets source and component folders, not the front-page hero. The check's `FIXED_ROOTS` omits `.` - a deliberate, documented refinement of the literal ADR allowlist.
2. **Quadrant READMEs are NOT emitted to the site.** `gen-docs-site.mjs` skips `README.md` (a folder guide is a repo-browsing aid, not a site content page; the sidebar already lists the quadrant's pages). They still carry the full D3 taxonomy so `G7` passes over them. `docs/` the root is not checked (only the four `docs/*` quadrants).
3. **`.gitkeep` is in `INVENTORY_SKIP`** (scaffolding, not content), so a folder that keeps a `.gitkeep` is neither required to list it nor flagged. `skills/` and `templates/` themselves are not allowlisted roots.
4. **`templates/seed-plugin/` is EXCLUDED.** Its `README.md` is the **seed payload** copied verbatim into a scaffolded plugin (the new plugin's README starter), not a folder guide; a folder-guide inventory there would pollute every scaffolded plugin. `templates/` lists `seed-plugin/` as one child. The check's `FIXED_ROOTS` omits `templates/seed-plugin`.
5. **README is never a component (latent bug fixed).** Adding `agents/README.md` exposed that the subagent/command enumeration (`listAgentFiles`/`listCommandFiles` in `fs-utils.mjs`) treated every `agents/*.md` / `commands/*.md` as a component, so a folder README became a bogus "README" subagent (tripping `prefix` S2, `components-index` S3, and `index-drift` G4). Fixed at the single enumeration point: `README.md` is excluded from component discovery everywhere.
6. **`INVENTORY_SKIP` is a superset of `SKIP_DIRS`.** The check reads the working tree with `readdirSync`, so the walk surfaces gitignored/VCS entries (`.git`, `_local`, `node_modules`); `INVENTORY_SKIP` reuses the shared `SKIP_DIRS` plus `README.md`, lockfiles, `.gitkeep`, and `.DS_Store`.
7. **The inventory parser (hardened after the P4 adversarial review).** `listed` is the set of FIRST backticked tokens of the list items under an inventory heading (`Inventory` / `Contents` / `Repository map` / `Repository structure`) - NOT the union of every backtick and link target the earlier draft contemplated. Scoping to inventory-section list items, and taking only the first backtick per item, is what makes phantom detection sound (a prose `node:fs` or a backtick inside a child's description never counts as listed). Two soundness fixes the review surfaced: (a) the section is **sticky** - a DEEPER sub-heading (e.g. `### Planned` under `## Inventory`) stays inside the inventory, so items under it are not silently dropped; only a same-or-shallower heading closes the section; (b) a README with **no inventory heading at all** is a finding, so a missing inventory is reported distinctly from a correctly-empty one.
