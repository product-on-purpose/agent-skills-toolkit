...
- `scripts/lib/standard-version.mjs` - `parseStandard()`, `compareStandard()`, `isAfter(since, pinned)`, and the `BASELINE` sentinel: the pure version arithmetic. `isAfter` is the predicate that answers "was this check introduced after the plugin's pin?", and it returns false for a missing or unparseable pin, so a garbage pin grades at full strength instead of silently disabling checks.
- `scripts/lib/standard-gate.mjs` - `SINCE_BY_REQ` (the reqId to `since` map, built once from the check registry) and `applyStandardDowngrade(findings, pinned)`: the downgrade pass itself, and the single source of the burndown arithmetic. It only ever rewrites `error` to `warn`, never the reverse.
- `scripts/check.mjs` - `runGate()`: applies the downgrade before computing the exit code, so it reaches both the terminal output and the exit status. Note that `--strict` bypasses the downgrade entirely, which is how a maintainer grades against the newest spine regardless of pin.
- `scripts/tier-report.mjs` - `defaultResolved()`, the default findings argument to `computeTierReport()`: applies the downgrade before the tier computation, so the tier label and the blocked list reflect the pinned-version ruleset.
- `scripts/evaluate.mjs` - `evaluate()` (plugin scope): applies the downgrade as part of the findings-resolution pipeline.
- Every `scripts/checks/*.mjs` module: each exports a `meta.since` field (a `"MAJOR.MINOR"` string or `"0.x"` for pre-policy checks) so `applyStandardDowngrade` can compare it against the pinned version. Six checks currently carry a `since` later than the baseline: `U12` (mermaid-valid) and `G7` through `G10` at 0.10, and `U13` (skill-registration) at 0.12.
- `templates/seed-plugin/library.json` - the `standard` field that `askit-init-plugin` copies verbatim into every plugin it scaffolds. This is the one site where honoring an older pin is always wrong: a plugin created today has no legacy to protect and must be born on the current Standard. `tests/unit/init-anatomy.test.mjs` asserts the seed pin equals the toolkit's own declared Standard. Omitting this site is how the seed sat at 0.8 through four Standard minors, granting every newly scaffolded plugin silent amnesty on all six checks above.

Grep anchor: `applyStandardDowngrade` (the function that carries the burndown contract).

> **Corrected 2026-07-27.** As first written, this section attributed `applyStandardDowngrade` to `standard-version.mjs`, described a standardGate(checks, pinned) wrapper that was never written, and omitted the seed template. `tests/unit/adr-implementation-sites.test.mjs` now asserts that every symbol named in an Implementation sites section resolves to real source, so a section cannot again point at something that does not exist.
