...
- **Status:** Accepted; a pre-release adversarial review found the advertised gating flip did not actually gate the harness's own central check, fixed in this same ADR - see the Addendum after Consequences.
...
> **Correction, 2026-08-11 (pre-release adversarial review), same day: "known to behave correctly" above was false.** The tests referenced in this paragraph exercised `decideExitCode`'s own `mode` branching correctly, but nothing exercised `anyDisagreement()` against a `metadata-parity` result - the harness's own central, novel check (see Context, "The parsed-values requirement") - at all. `anyDisagreement()` as it shipped when this paragraph was written excluded `metadata-parity` from gating UNCONDITIONALLY, even after `PARITY_MODE` moved to `"gating"`. Concretely: the seeded-violation demonstration earlier in Context (`agentskills validate` PASS, parsed-values `[MISMATCH]`) would **not** have blocked a gated run on that exact fixture - `PARITY_MODE = "gating"` would still have exited `0`. The "one-line promotion to gating" this ADR advertises was therefore not real for the one disagreement class the harness was built to detect. Fixed in this same ADR; see the Addendum after Consequences for the finding, the fix, corrected verbatim evidence, and a sibling-exclusion audit. Left in place rather than edited away: the claim being wrong is part of the record.
...
## Addendum (pre-release adversarial review, 2026-08-11): the gating flip did not gate its own central check

**Found by adversarial review, before this release tagged - not by this ADR's author.** The finding, verbatim: "The advertised gating flip ignores parsed-value disagreements. `anyDisagreement()` in `scripts/check-parity.mjs` explicitly excludes `metadata-parity` failures even after `PARITY_MODE` changes to `"gating"`. So the reference parser can mangle a `metadata` value while the vendor validator exits 0, and the supposedly gated harness still exits 0. That is the exact disagreement class the harness exists for."

**Why this is the worst possible place for this bug.** ADR 0040's whole argument - the reason this ADR's parsed-values requirement exists at all - is that `agentskills validate`'s exit code is not sufficient evidence: it reported "Valid skill" for all 24 skills while `metadata.chain` was being silently mangled. This ADR's own metadata-parity check exists specifically because exit codes cannot be trusted for that class of defect. Excluding that exact check from gating reproduces the metadata.chain incident's blind spot one level up: a harness built to distrust one exit code, trusted unconditionally itself.

**How it got through.** The original `anyDisagreement()` docblock stated its scope directly and deliberately: "the metadata-parity and pin-skew sections are evidence, not first-party verdicts, and are never gating even after PARITY_MODE flips." That sentence was written on purpose, not missed by accident - the reasoning behind it simply does not survive scrutiny. Grouping `metadata-parity` with pin-skew hid the difference between the two: pin-skew genuinely is non-gating evidence by design (it compares two identities this ADR itself documents as not expected to agree), while `metadata-parity` IS a first-party verdict in every sense that matters - the reference implementation's own parser produced the value being compared. No test caught the gap because no test exercised `anyDisagreement()` against a `metadata-parity` result at all; the suite covering the gating flip tested `decideExitCode`'s mode branching and the `vendor-validate`/`exception-integrity` kinds, and stopped there. The gap was found only when a reviewer read `anyDisagreement()` against the class of defect it was supposed to detect, rather than against the classes it already handled correctly - the same generalizable lesson ADR 0038 recorded ("grep for every site a decision constrains, not just the one that prompted it"), recurring here one level up: this ADR's own gating logic needed the same audit its parsed-values check gives every skill's `metadata:` block.

**The fix.**
1. `anyDisagreement()` now counts a `metadata-parity` result whenever `pass !== true` - an executed mismatch (`pass: false`) and the section being unable to run at all (also `pass: false`, see point 2) BOTH count. There is deliberately no exception/suppression path for this kind, unlike `vendor-validate`: a corrupted parsed value is never a legitimate difference of opinion with a vendor to document away the way a missing placeholder `author` is (see ADR 0043) - it is always a real defect, so "unsuppressed" is true by construction here, not by an override mechanism.
2. `metadataParityUnavailableResult()` is a new pure helper: when `uvx` is not on PATH, Section C now pushes this result (`pass: false`, never `null` or omitted) instead of pushing nothing. Before this fix, an unavailable reference parser produced ZERO `metadata-parity` results, which - even with fix 1 alone - would have let a missing `uvx` silently clear a gated run. This is the "fail closed when the reference parser cannot run" requirement: a harness that goes quiet about its own most important check must not thereby report a clean parity result, the same lesson the metadata.chain incident (silence, not a wrong answer) already taught once.
3. A test-only `--mode=` CLI override (parsed in `main()`, e.g. `node scripts/check-parity.mjs . --mode=gating`) lets `tests/unit/check-parity.test.mjs` drive the gating branch end-to-end without editing the shipped `PARITY_MODE` constant, so the default stays `"report-only"` for every real invocation and every other test. Two new CLI tests: a real seeded metadata-parity mismatch (the identical fixture mutation used in Context's original demonstration) exits `0` under report-only and `1` under `--mode=gating`; and `--mode=gating` against the real, unmodified repository (one documented exception, nothing undocumented) still exits `0`, proving the override does not fail merely by being engaged.

**Corrected verbatim evidence**, replacing the implicit claim in Decision outcome that the original seeded-violation demonstration proved the gating branch worked - it proved only `decideExitCode`'s own arithmetic, never that `anyDisagreement()` would hand it a `true` for this class. Same fixture as Context's original demonstration (`skills/askit-decision/SKILL.md`'s `metadata.audience` changed to a YAML block-sequence list), run under both modes, then reverted via `git checkout --`:

```
$ node scripts/check-parity.mjs .                        # report-only (the shipped default)
  [MISMATCH] skills/askit-decision
    metadata.audience: coerced-non-string - ours=["advanced","beginner"] reference="['advanced', 'beginner']"
  metadata-parity: 1 finding(s) (a real mismatch, or the section being unable to run at all) - WOULD
  block once gating starts; there is no exception path for this kind.
  PARITY_MODE: report-only -> exit code 0 regardless of the findings above.
$ echo $?
0

$ node scripts/check-parity.mjs . --mode=gating          # test-only override, proving the fixed flip
  [MISMATCH] skills/askit-decision
    metadata.audience: coerced-non-string - ours=["advanced","beginner"] reference="['advanced', 'beginner']"
  metadata-parity: 1 finding(s) (a real mismatch, or the section being unable to run at all) - WOULD
  block once gating starts; there is no exception path for this kind.
  PARITY_MODE: gating -> exit code 1 regardless of the findings above.
$ echo $?
1
```

**The sibling-exclusion audit this same review round asked for.** Every `kind` a `results` entry can carry was re-read against `anyDisagreement()`, specifically looking for another case grouped with `metadata-parity` under the same wrong reasoning:
- `vendor-validate` (claude / skills-ref exit-code results) - correctly excluded when `ran: false` (tool unavailable). Unlike `metadata-parity`, this kind has a real fallback path for `claude` (`vendorValidateManifest`) and CI always installs both real tools (see the tooling-installability evidence in Context), so "could not check" is a genuine, survivable environment gap for this kind, not the harness's central claim going unverified. No change made.
- `exception-integrity` - already counted unconditionally on `pass === false` before this review. No gap found, no change.
- `metadata-parity` - the reported defect. Fixed, per above.
- Pin-skew (Section D) - never pushed into `results` at all, and correctly so on inspection: it compares a git-blob-SHA source pin against an installed PyPI release, two identities this ADR's own Context documents as not expected to always agree ("These are not expected to always agree... so the harness reports both and flags skew rather than asserting equality"). A skew is evidence to report, never a first-party rejection to gate on - the one case in this audit that was already right for the right reason.

No fourth kind exists in `scripts/check-parity.mjs` today. This audit is recorded here, in the document, rather than left as only a code comment, because "check whether a sibling exclusion exists" is exactly the follow-through step this repository's own process notes (ADR 0038; ADR 0041's round-2 addendum) record as the step that gets skipped when a fix stops at the one reported instance.

**Status of this ADR:** Accepted, corrected in place per the above rather than silently rewritten - the TL;DR Status line and the inline correction in Decision outcome both point here. The claim being wrong is part of the record.

## Implementation sites
- `scripts/check-parity.mjs` - `PARITY_MODE`, the one-line gating flip; `diffMetadataParity()`, the parsed-values comparison (the requirement this ADR exists to satisfy); `gitBlobSha1()`, the pure git-blob-sha1 formula used for pin-skew comparison; `summarizePinSkew()`, the pin-vs-installed identity comparison; `vendorValidateManifest()`, the reduced-fidelity local-only fallback; `decideExitCode()` and `anyDisagreement()`, the gating-flip logic - see the round-2 addendum sites below for the correction to `anyDisagreement()`'s coverage; `runReferenceParse()` and `REFERENCE_PARSE_SCRIPT`, the reference-PARSER invocation (not the validator) that the parsed-values check depends on; `runVersionProbe()` and `VERSION_PROBE_SCRIPT`, the validator-version and pin-identity probe; `readOurMetadata()`, this repository's own side of the parsed-values comparison; `main()`, the orchestration and report renderer.
...
**Round-2 addendum sites (the gating-flip fix, see Addendum above):**
- `scripts/check-parity.mjs` - `anyDisagreement()`'s corrected docblock and body: the `metadata-parity` branch (`r.pass !== true`, no exception path) and the `exception-integrity` branch, contrasted per-kind against `vendor-validate`'s different, intentional rule.
- `scripts/check-parity.mjs` - `metadataParityUnavailableResult()`, the new pure helper Section C pushes (`pass: false`, never `null`/omitted) when `uvx` is not on PATH, so an unavailable reference parser fails closed instead of producing zero results.
- `scripts/check-parity.mjs` - `main()`'s `--mode=` CLI override (the `modeArg`/`mode` local variables), a test-only escape hatch that never changes the shipped `PARITY_MODE` constant.
- `tests/unit/check-parity.test.mjs` - the corrected `anyDisagreement` coverage (an executed metadata-parity mismatch counts; a passing one does not; `metadataParityUnavailableResult()` counts; an unrecognized kind does not) and `metadataParityUnavailableResult()`'s own contract test; two new CLI tests proving the real seeded fixture exits `0` under report-only and `1` under `--mode=gating`, and that `--mode=gating` alone does not fail a clean run.

Grep anchor: `PARITY_MODE` in `scripts/check-parity.mjs` (the flip point). For the parsed-values requirement specifically: `diffMetadataParity` in the same file and in `tests/unit/check-parity.test.mjs`. For the documented-exception path: `PARITY_EXCEPTIONS` in the same file. For the round-2 gating-flip correction: `metadataParityUnavailableResult` in the same file.
