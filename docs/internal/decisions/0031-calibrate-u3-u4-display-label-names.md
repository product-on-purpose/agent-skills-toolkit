...
TDD added 9 tests. In `tests/unit/frontmatter-valid.test.mjs` (5): display-label skip, colon-namespace skip, malformed-canonical still errors, the overlong `name === dir` length branch still errors, and a display-label still gets the description check. In `tests/unit/name-matches-dir.test.mjs` (4): U4 warn under plain-plugin, U4 error under askit-library, an end-to-end `runGate` exit-code assertion (a name-mismatch gates under the default ladder, exit 1, but only warns under plain-plugin, exit 0), and a raw-check cross-test that the colon-namespace input U3 now skips is still caught by U4. (The last three were added to close coverage gaps the pre-merge adversarial review identified: the length branch, the user-visible exit-code claim, and the "U4 still owns the divergence" safety argument.) Suite 366 -> 375; gate Advanced 0/0; spine 29; Standard 0.11.

Verified against the corpus under `--profile plain-plugin` (a local fixture reproducing the saved corpus name shapes; see `_local/audit/anchor-runs/2026-06-10/FINDINGS.md`): `hookify` goes from 1 name error to 0 (one U4 warning - its name `writing-hookify-rules` is valid kebab, so only U4 fired, never U3), and `gsd` from 165 errors to 1 (82 U4 warnings, 0 U3 format errors). The single remaining gsd error is a genuine YAML parse defect in `skills/debug/SKILL.md` (an unquoted flow sequence in the description), which is not a name-format issue and correctly stays an error - a useful confirmation that the calibration suppresses the false positives without masking a real parse defect. The toolkit's own skills are unaffected (every askit skill name is kebab and equals its directory).
