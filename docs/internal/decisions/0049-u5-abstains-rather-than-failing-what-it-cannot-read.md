...
- **ADR 0048 (a command is not a skill)** - the sibling. Both ADRs are the same underlying defect measured on a different population, and they are deliberately independent so either can be ratified without the other. See "The shared root cause" in ADR 0048.
