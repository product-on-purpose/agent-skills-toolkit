...
- **Declared tier:** Advanced (Gold). `library.json` `tier: advanced`, `standard: 0.10`, version `1.0.0`. Self-proving: the toolkit passes its own Gold gate.
- **Self-validating:** yes. `node scripts/check.mjs` exits 0 (0 errors, 0 warnings); `tier-report` prints Advanced with an empty burndown (satisfies universal + convergent + advanced); `npm test` = 228/228 (verified 2026-06-03).
...
- **ADR 0024 v1.1.0 docs build-out (in progress):** the staged author-before-enforce program (`release-plans/plan_v1.1.0/`) taking the spine 25 -> 30 (U12 + G7-G10) and the Standard to v0.10. MERGED to `main`: P0 (demonstrative no-dash hook, G1 non-vacuous; #88), P1 (full Diataxis content + frontmatter taxonomy on all docs pages; #89), P2a (`docs-frontmatter` G7 + Standard v0.10; #90), P2b (the generated Pattern S site; #91), and the HTML evaluation-report sample templates + E1 spec (#92). Remaining: P3 (README hero + `mermaid-valid` U12), P4 (folder-README G8), P5 (source-doc G9), P6 (docs-presence G10 + the G7 renumber close-out + STANDARD v0.10 normative additions + the 1.1.0 version bump + marketplace re-pin).
