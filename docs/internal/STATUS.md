...
- **Declared tier:** Advanced (Gold). `library.json` `tier: advanced`, `standard: 0.11`, version `1.5.0`. Self-proving: the toolkit passes its own Gold gate.
- **Self-validating:** yes. `node scripts/check.mjs` exits 0 (0 errors, 0 warnings); `tier-report` prints Advanced with an empty burndown (satisfies universal + convergent + advanced); `npm test` = 358/358 (verified 2026-06-09).
