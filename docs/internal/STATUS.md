...
**Going public (2026-06-01, maintainer go):** the repo is being flipped public at Silver (the planned 0.x preview), with `v1-build` merged to `main`, branch protection on `main`, and an updated repo description. The live Astro Starlight docs site is built (`site/`, `npm run build` green, 7 pages) and wired to GitHub Pages via `.github/workflows/deploy-pages.yml`.
