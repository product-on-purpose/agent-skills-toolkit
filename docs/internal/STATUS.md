...
| Skills | **26** |
| Tests | **1464, 0 failures** (1 skipped; local suite run **2026-09-02**). It read **1446 at `4ba1ae6` (v1.17.1)**; of the eighteen added since, twelve are E57's, covering the RELEASE-NOTES section gate: the tagged-and-published tree captured as a fixture, its repaired twin, the extraction rules the awk used to hold, and the assertions that both callers stay wired to one implementation, and six are RS-F3's, asserting that the new standards-watch schedule cannot quietly become an aspiration again. Earlier: **1439 at `fd5286b` (v1.17.0)**, **1399 at `1da4d16` (v1.16.1)** and **1359 at `9133014` (v1.15.0)**, each confirmed there by `npm run release-ready` exiting 0 on the release runner |
