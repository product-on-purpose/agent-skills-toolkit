...
- **Self-validating:** yes. `node scripts/check.mjs` exits 0 (0 errors, 0 warnings); `tier-report` prints Advanced with an empty burndown (satisfies universal + convergent + advanced); `npm test` = 280/280 (verified 2026-06-03 on `main`).
- **Installable:** yes - **SHIPPED `v1.1.0` (2026-06-03).** Tagged at commit `f3250c0` (`release.yml` minted the GitHub release behind the version-consistency guard; live + Latest), and the `product-on-purpose` marketplace re-pinned to it (`agent-plugins` PR #14, `metadata 1.7.0`, entry version 1.1.0); the install resolution path was smoke-verified end to end. Supersedes `v1.0.0` (2026-06-02, PR #84, commit `f5291c0`, registry PR #9). Install: `/plugin marketplace add product-on-purpose/agent-plugins` then `/plugin install agent-skills-toolkit@product-on-purpose`; an existing install updates from the marketplace.
