...
2. **Same-target cross-model A/B.** The current matrix mostly confounds target with model (each cell used a different target, chosen deliberately to widen coverage first). To isolate the model/effort effect, run two or three models on the SAME target and diff their finding sets against the scoring key (or against a verified union). The first full triple exists: R9/R10/R11 (Opus, Sonnet, Haiku, all at high effort with an identical prompt on the same pm-toolkit plugin; readings 14 and 16). It showed the tiers differ in finding KIND, that the categorical boundary is Haiku-vs-frontier (Haiku confabulated a correction even at high effort), that Sonnet/high matched Opus/high here at ~70% cost, and that the two frontier models' verified finding sets only partially overlap - so a two-frontier panel beats either alone. Single-target caveat: replicate on a structurally-defective target before hardening the Opus-vs-Sonnet parity into guidance.
