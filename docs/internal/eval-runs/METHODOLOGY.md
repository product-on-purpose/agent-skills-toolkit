...
2. **Same-target cross-model A/B.** The current matrix mostly confounds target with model (each cell used a different target, chosen deliberately to widen coverage first). To isolate the model/effort effect, run two or three models on the SAME target and diff their finding sets against the scoring key (or against a verified union). The first instance exists: R8 vs R9 (Haiku/medium vs Opus/high on the same pm-toolkit plugin, reading 14) - it showed the tiers differ in finding KIND (knowledge-content errors were Opus-only), which is exactly the effect a confounded matrix cannot see.
