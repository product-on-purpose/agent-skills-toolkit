...
- **A command's `description` is a LABEL, not a trigger surface, and is not held to the sec 8.1 discoverability bar.** A skill is selected by a model matching its description against a request, so for a skill the description IS the trigger surface. A command is selected by a person typing `/<name>`, and Claude Code shows the description as the label beside it; on Codex there is no command artifact at all (see the parity note below), so the backing skill's description does the triggering and the command's does not exist. **On neither runtime does a command description perform trigger matching**, so applying a trigger-quality bar to it measures a property the artifact does not have. This was measured before it was written: **0 of 14 commands across the reference family satisfy the sec 8.1 bar, including this toolkit's own two - whose backing skills, of identical name and intent, score 1.00** (ADR 0048). The agreement obligation above is therefore a SHOULD: it is real, and it is not machine-checkable.
