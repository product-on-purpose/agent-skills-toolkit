...
> **Standard version 0.16**, amended 2026-09-05 (promoted to the repository root at v0.8 on 2026-05-26). This is the normative Standard that every tool in `agent-skills-toolkit` enforces; the version-history notes below record how the frozen draft converged.
> v0.16: **one check added and two existing checks tightened, all three warn-first and all three capped until 0.17.** The spine moves from 34 to **35**. **`U18` (`command-size-cap`, sec 3.2, ADR 0058)** is the new one: on a plugin that declares `codex` as an agent-target, a `commands/*.md` over the byte cap Codex applies when it migrates commands into skills is a `warn`. The vendor **SKIPS** an oversized command rather than truncating it - `codex-rs/core-plugins/src/command_migration.rs` does `continue`, so no skill is written and no error is raised - which makes this the only thing the spine grades whose symptom is a component's ABSENCE. The check measures the source file as a **declared proxy** for the rendered skill the vendor actually caps, and that is why it is a `warn` rather than an `error`: an error would assert a measurement it did not take. **`G1` (hook documentation)** now reports a hook handler type Codex parses and silently skips, and **`G2` (self-hosting CI)** now credits a gate that is EXECUTED rather than merely named; both shipped in the same train. **All three carry `until: "0.17"` and never `until: "0.16"`**, because `scripts/lib/standard-ceiling.mjs` gates on `isAfter(migration.until, pinned)`: a cap expiring at the revision that introduces it is inert for every adopter of that revision, which would give a plugin adopting 0.16 zero migration window. `U18` additionally carries `since: "0.16"`, because `until` alone would leave a plugin pinned below 0.16 exposed to a check that did not exist at its pin. **Blast radius was measured for all three, not argued:** the six reference-family members were graded at the catalogue's pinned shas before and after each change, with zero movement. The `G2` tightening is additionally recorded as having had a WRONG first version - a rule that blanked quoted runs moved a member from passing to failing, and that member's CI was correct - which is why the measurement is the rule and the argument is not.
> v0.15: **two windowed requirements graduate from `warn` to gate-failing `error`, and nothing else changes.** No check is added, no check is removed, and the spine stays at **34**. The **workflow half of the components mirror** (sec 3.4, ADR 0047) and **`U17` (`catalogue-manifest-shape`, sec 12, ADR 0052)** were both introduced at 0.14 carrying a finding-level `migration.until` of `0.15`; that cap now expires, so a plugin pinned at 0.15 takes an `error` where a plugin pinned at 0.14 or below still takes a `warn`. **Per sec 7.7 this is the burndown completing, not a new requirement**: nothing moves red-ward for a consumer who does not re-pin, and no code changed to make it happen, because the graduation is data in the finding's migration metadata rather than a hand-edited constant (ADR 0044). Both graduations were measured before ratification and **neither produces a single finding on any of the six reference-family members at their own pins.** The workflow mirror's graduation is additionally evidenced by its own subject: the member that would have lost a tier to it declared its nine workflows on 2026-08-15, one day after ADR 0047 was ratified and inside the window that ADR created, which is a warn-first migration observed doing its whole job. `U17`'s graduation was taken against a re-run census that still shows **zero mixed and zero malformed manifests**, and ADR 0052's dated addendum records why graduating a preventive check is nevertheless the right call rather than extending a window with no terminating condition.
> v0.14 (amended 2026-08-15): sec 3.2's statement of WHY a command description is not a trigger surface is corrected. It reasoned that a command is not a skill; Claude Code states that *"Custom commands have been merged into skills"* and that the `commands/` directory holds *"skills as flat Markdown files"*. **A command IS a skill** - one whose invocation is user-controlled - and that is the real reason its description does not perform model-facing trigger matching. The RULE is unchanged; only its justification is. ADR 0048 is amended in place and carries the full correction.
> v0.14: **three checks added and one normative MUST demoted**, all from the v1.14.0 ADR pack (ADRs 0046 to 0052), and every one of them measured before it was written.
> `U15` (`agents-dir-registerable`, sec 3.3, ADR 0046): every `.md` under `agents/` MUST be a registered subagent. Claude Code registers **every** file it finds there - a probe registered `README` and `_README` as subagents - so a file excluded from the plugin's own registration is still live, and escapes every check that reads the registration list. The obvious alternative (pointing the four Silver checks at the runtime list) was prototyped and **rejected**: its remediation tells the author to declare `agents/README.md` as a subagent, creating the phantom the `G8` `agents/` exemption exists to prevent. `U16` (`metadata-placement`, sec 3.7, ADR 0050): a sec 3.7 governance key declared at the TOP LEVEL, where nothing reads it. The frontmatter **vocabulary is now stated as OPEN** in sec 3.8 - strictness was rejected because 44.9 percent of 2342 measured skills carry a key this Standard does not name, and sec 3.7 calls the upstream `metadata` map arbitrary. `U17` (`catalogue-manifest-shape`, sec 12, ADR 0052): a `marketplace.json` that is unparseable, has no `plugins` array, or MIXES skill-source and plugin-source entries, so that no scope reads it or only half of it is read. **`U17` is `warn` at both 0.13 and 0.14 and becomes a gate-failing `error` at 0.15**, because a census found zero real instances of either shape: it is preventive, not corrective.
> v0.14 (continued): also at 0.14, with no new check: the **workflow half of the components mirror** (sec 3.4, ADR 0047) - a `_workflows/<name>.md` on disk MUST be declared in `components.workflows` and vice versa, introduced here as a `warn` and gate-failing at **0.15**; **sec 3.2's command-description MUST is DEMOTED to a SHOULD** (ADR 0048), because it was unenforceable as written and 0 of 14 commands in the reference family satisfied it, this toolkit's own two included; and **sec 8.1 is scoped to skills and its scorer now DECLINES rather than fails a language it cannot read** (ADR 0049), after the bar was found unpassable in French by construction. Sec 12 additionally records that a catalogue-level finding is never a numbered requirement laid on an individual plugin (the unilateral-remedy test, ADR 0051), which **closes** the question of graduating cross-member findings. Per sec 7.7 a plugin pinned below 0.14 sees a `warn` for `U15` and `U16` and adopts them by re-pinning; `U17` and the workflow mirror are `warn` for every pin below 0.15. The Universal checks are now `U1-U9`, `U11-U17`; the spine is `U1-U9` + `U11-U17` + `S1-S8` + `G1-G10` = **34**.
>
> v0.13: `U14` (`agent-restricted-fields`) added to the Universal tier (ADR 0045). An agent shipped inside a plugin MUST NOT declare `hooks`, `mcpServers` or `permissionMode`: Claude Code refuses these on plugin-shipped agents **for security reasons**, in the vendor's own words, so an author who writes one has configured something the runtime will not honour and receives no signal that it was refused. Detected across catalogue members since v0.12 as the marketplace `A6` reading; promoted to a numbered requirement here so a plugin graded on its own is told too. Per sec 7.7 a plugin pinned below 0.13 sees a `warn`; adopting 0.13 is what makes it gate. `U13` (`skill-registration`) graduates to a gate-failing `error` at this version, as scheduled by ADR 0035, and the string-shaped `S4` chain-declaration cap lifts, as scheduled by ADR 0041. The Universal checks are now `U1-U9`, `U11-U14`; the spine is `U1-U9` + `U11-U14` + `S1-S8` + `G1-G10` = **31**.
>
> v0.12: `U13` (`skill-registration`) added to the Universal tier (ADR 0035). A plugin whose manifest enumerates skills MUST register every skill it ships on disk, and every registered skill MUST exist on disk - a skill on disk but unregistered ships invisibly to installers, a registered skill with no directory cannot be delivered. This is the first Standard growth since the 0.11 relaxation; per sec 7.7 it ships as a `warn` for the 0.12 minor (the first live use of the burndown) and becomes a gate-failing `error` at 0.13. The Universal checks are now `U1-U9`, `U11-U13`; the spine is `U1-U9` + `U11-U13` + `S1-S8` + `G1-G10` = **30**.
...
- Register every skill it ships: where the plugin's manifest enumerates skills (`library.json` `components.skills`, or a `.claude-plugin/marketplace.json` `plugins[]` catalog whose entries point under `skills/`), every skill directory on disk MUST be registered, and every registered skill MUST exist on disk (`U13`, `skill-registration`; conditional on the plugin carrying an enumerating manifest). A skill on disk but unregistered ships invisibly to installers; a registered skill with no directory cannot be delivered. This is introduced at Standard 0.12 and, per sec 7.7, ships as a `warn` for the 0.12 minor before it becomes a gate-failing `error` at 0.13.
- Ship no agent that declares a field the runtime refuses: an agent under the plugin's `agents/` directory MUST NOT declare `hooks`, `mcpServers` or `permissionMode` (`U14`, `agent-restricted-fields`). Claude Code's plugin reference states, verbatim: "For security reasons, `hooks`, `mcpServers`, and `permissionMode` are not supported for plugin-shipped agents." The field is refused rather than ignored, and nothing tells the author, so what they believe they configured is simply not in effect. Introduced at Standard 0.13; a plugin pinned below 0.13 sees a `warn` until it adopts 0.13. Applies to every agent the plugin ships, independent of its declared `agent-targets`, because a consumer can install any plugin into Claude Code and `agent-targets` is a declaration of intent rather than an installation constraint (ADR 0045).
...
- **Rules:** a command MUST map to exactly one skill or workflow. Its `description` MUST be non-empty and state what invoking the command does, and it SHOULD agree with the backing skill's intent. For multi-target plugins, a command MUST be emitted in each target's format (CC: `commands/<name>.md`; CX: the backing skill, explicitly invocable).
- **A command's `description` is a LABEL, not a trigger surface, and is not held to the sec 8.1 discoverability bar.** The reason is INVOCATION CONTROL, not component type. Claude Code states that *"Custom commands have been merged into skills"* and that a `commands/*.md` file and a `skills/<name>/SKILL.md` *"both create `/deploy` and work the same way"*; the `commands/` directory holds *"skills as flat Markdown files"* (read 2026-08-15). **A command is therefore a skill** - one whose invocation is user-controlled. What distinguishes it is that a model does not load it automatically, so its description is not performing model-facing trigger matching, and applying a trigger-quality bar measures a property the artifact does not exercise. The same runtime exposes `disable-model-invocation` and `user-invocable` frontmatter to declare that control explicitly. On Codex there is no command artifact at all (see the parity note below), so the backing skill's description does the triggering. Measured before this was written: **0 of 14 commands across the reference family satisfy the sec 8.1 bar, including this toolkit's own two - whose backing skills, of identical name and intent, score 1.00** (ADR 0048, as amended 2026-08-15). The agreement obligation above is therefore a SHOULD: it is real, and it is not machine-checkable.
- **A command a target's migration would DROP is a defect of the plugin, not of the target.** On a plugin that declares `codex` as an agent-target, every `commands/*.md` SHOULD stay under the byte cap at which Codex's command migration refuses it (`U18`, `command-size-cap`). Codex migrates a plugin's commands into skills and **skips** any whose rendered skill exceeds `MAX_MIGRATED_COMMAND_SKILL_BYTES` (4000 bytes, read 2026-09-05 from `openai/codex` at `52e73e3a`): the loop moves on, no `SKILL.md` is written, and no error is raised anywhere. The command therefore does not exist on Codex, and absence has no symptom - which is why this is the requirement rather than a note. The cap applies to the RENDERED skill, so the check measures the source file as a declared proxy and reports at `warn`; asserting an exact rendered size would be claiming a measurement it did not take. Introduced at Standard 0.16 and, per sec 7.7, held at `warn` through 0.16; it gates from 0.17 (ADR 0058).
...
- **Every `.md` file under `agents/` MUST be a registered subagent** (`U15`, `agents-dir-registerable`). Claude Code discovers subagents by scanning `agents/` for `*.md` and registers **every file it finds**: a directory holding `real-agent.md`, `README.md`, `_README.md` and `README.txt` registers three subagents, `real-agent`, `README` and `_README`. The underscore prefix protects nothing; only the non-`.md` extension is skipped. A file the plugin excludes from its own registration is therefore still a live subagent, with whatever name and frontmatter it happens to carry, and it escapes every check that reads the registration list. There are exactly two conformant resolutions and neither is "register the folder guide": **register the file** (give it the plugin prefix and declare it in `library.json` `components.subagents`), or **move it out of `agents/`**. Folder documentation for `agents/` belongs in `AGENTS.md` or the root README's component table; for the same reason, `agents/` is excluded from the meaningful-folder README requirement (`G8`, sec 2.6). Introduced at Standard 0.14; a plugin pinned below 0.14 sees a `warn` until it adopts 0.14 (ADR 0046).
...
- **CX:** a smaller event set of **12** (read 2026-09-04): `SessionStart`, `SessionEnd`, `SubagentStart`, `PreToolUse`, `PermissionRequest`, `PostToolUse`, `PreCompact`, `PostCompact`, `UserPromptSubmit`, `SubagentStop`, `Stop`, `Interrupt`. Handler types are **not** the Claude set: only `command` and `mcp_tool` execute. The vendor's words are *"`command` and `mcp_tool` handlers are supported. `prompt` and `agent` handlers are parsed but skipped"* - so a `prompt`- or `agent`-type hook shipped to Codex is a silent no-op, not an error.
...
- `version` (string, semver), `updated` (date), `tier` (`universal`|`convergent`|`advanced`), `audience` (`beginner`|`intermediate`|`advanced`), `category` (string), `agent-targets` (list), `status` (`active`|`deprecated`|`experimental`), `deprecated-by` (component name, when status is deprecated), `remove-in` (target plugin version), `verified-against` (map of agent name to the agent version this component was last CHECKED against, ADR 0054).

**`verified-against` records a READING, not a correctness claim**, and it is OPTIONAL at every tier. It means a human checked the component against that agent version; it asserts nothing about versions since, and a component whose recorded version predates a platform change is a prompt to look rather than a defect. It is deliberately unchecked: the useful assertion - that somebody actually read the component - is not machine-checkable, and a check for a field nobody populates would measure adoption of the check rather than quality (ADR 0054).

**These keys live under `metadata` and are not read at the top level.** A key named above, declared at the top level of a component's frontmatter instead of inside the `metadata` map, MUST be reported (`U16`, `metadata-placement`). Nothing reads it there, so an author who wrote a `version` at the top level has not recorded one and receives no signal either way - the same silent no-op class as `U14`. Where the key appears in both places the nested one is read and the top-level copy is dead weight that can drift from it. Introduced at Standard 0.14; a plugin pinned below 0.14 sees a `warn` until it adopts 0.14 (ADR 0050).
...
- **Common to all components:** `name` (kebab-case, matches file/dir), a non-empty `description`, plus applicable Section 3.7 metadata keys. **What that description must ACHIEVE differs by component type, and Section 8.1 is not universal:** a SKILL is selected by a model matching its description against a request, so a skill description MUST satisfy the Section 8.1 discoverability bar (what + when + trigger keywords). A COMMAND is invoked by name, so its description is a caller-facing label and is explicitly NOT held to that bar (sec 3.2). For every other component type the requirement is the non-empty statement of what the component does; no trigger-quality bar applies.
...
- **Rules:** frontmatter MUST be valid YAML; required keys MUST be present; `description` MUST be non-empty, and a SKILL's description MUST additionally satisfy the discoverability bar (8.1). **The bar is scoped, not universal.** An earlier draft of this section made 8.1 mandatory for every component while sec 3.2 and sec 8.1 both excluded commands, which let a command simultaneously conform to and violate this document. A component-wide requirement that a component-specific section contradicts is a defect in this Standard, not a choice for a reader to resolve.
- **The vocabulary is OPEN.** A key beyond those named here is PERMITTED at every tier, and is neither validated nor interpreted by this Standard. A plugin MAY carry its own keys at the top level or inside `metadata` without penalty. This is stated as a rule rather than left as silence, because silence reads as an unfinished thought: strictness was considered and rejected on measurement (44.9 percent of 2342 skills across thirteen sources carry a top-level key this Standard does not name, and 58.2 percent carry an unnamed `metadata.*` key), and because sec 3.7's own opening calls the agentskills.io `metadata` map **arbitrary** - a rule rejecting unknown keys inside it would put this Standard in contradiction with the spec it profiles. What IS constrained is PLACEMENT of the keys this Standard does define (sec 3.7, `U16`). See ADR 0050.
...
| `selfValidation` | `"vendored"` \| `"npx"` | OPTIONAL | Which self-validation command a generated `INDEX.md` names. **Absent means `"npx"`**, which is correct for every plugin that consumes a conformance toolkit rather than vendoring one; `"vendored"` declares that the plugin ships the gate itself. A CLOSED enum, not a command string: the generator owns the command text for each case and the manifest only selects between them, so a plugin cannot write arbitrary text into its own generated index. A present-but-unrecognised value is a finding and is not honoured - generation falls back to `"npx"`. Introduced at Standard 0.13. |
...
**Tightenings are pin-driven too, and the ceiling is ONE mechanism (MUST).** A requirement that already existed but is being made stricter - a `warn` promoted to `error`, or a rule newly reading a shape it previously ignored - carries a per-finding migration declaring the version at which the tightening takes effect. Until a plugin pins that version, the finding is held at the declared cap. Introduction (`since`) and tightening (`until`) are two INPUTS TO ONE CEILING, applied after all consumer configuration resolves; they are not two mechanisms, and a tightening is never applied on a calendar. **A graduation therefore happens when a plugin ADOPTS the version, not when the tooling ships it.**

**A tightened SUBRULE needs its own migration (MUST).** A rule added under an existing requirement inherits that requirement's `since`, which describes when the CHECK appeared and says nothing about when the rule inside it did. Without finding-level migration metadata such a rule would take effect immediately for every plugin, whatever it pinned - the exact failure this section exists to prevent.

**Consumer configuration cannot outrank the pin (MUST).** The ceiling is applied AFTER a consumer's profile, per-rule overrides and suppressions resolve, so a local override cannot raise a finding above what the pinned version permits. A grader MUST report why, rather than letting the override appear silently ignored.
...
**Consumer-side configuration (non-normative for conformance).** A grader MAY accept consumer-side configuration (a local `askit.config.json` that selects a named profile, sets per-rule severities, and lists a suppressions baseline) to scope HOW it grades. This configuration is the grader's local preference, NOT part of the plugin contract: a plugin is conformant or not independent of how any grader is configured. A PUBLISHED conformance verdict MUST NOT let the graded SUBJECT weaken an objective or vendor-cited finding to dodge it. **This is stated in terms of WHO chose the setting, and it changed at Standard 0.13.** Through 0.12 a published verdict merely clamped a subject's `off` up to a `warn`, which meant turning the mode on could never fail a passing gate; from 0.13 a subject-owned reduction is discarded entirely and the finding returns to the severity a grader-selected rubric would give it, so a published verdict CAN now fail where it passed. A subject may still be STRICTER about itself, and settings the GRADER supplied are honoured in full. A subject-owned suppression is likewise cleared rather than merely surfaced, because a waiver and a lowered severity are two independent ways to dodge the same finding. This profile and suppression mechanism is also the opt-in home for house preferences a tier requirement should not impose (the home ADR 0028 named for the retired no-dashes rule).
...
**SCOPE: this bar applies to a SKILL's description.** It is the bar `U5` scores, and `U5` scores skills only. A skill is selected by a model matching its description against a request, so the description IS its trigger surface and trigger quality is the right thing to measure. A **command** description is a label beside `/<name>` typed by a person and is explicitly out of scope (sec 3.2). Whether the bar extends to any other component type is an open question and this section does not assume it does.

**The scorer reads English and DECLINES rather than fails outside it.** Tooling implementing this rubric MUST NOT report a description as failing the bar when it cannot read the description's language: a low score would be a claim about the description, and only "not scored" is a claim the tool has evidence for. The reference implementation measures English function-word density and emits nothing below a floor, reporting the count of declined descriptions separately so a decline is never mistaken for a pass. The measurement behind this: the reference scorer's English trigger-phrase pattern fired on **0 of 346** parseable descriptions in a French corpus in which **341 carried an explicit French trigger clause**, and because that pattern is worth 0.35 of a 1.00 score against a 0.70 threshold, the bar was **unpassable in French by construction, at any quality** (ADR 0049).
...
- **One manifest, one kind.** A `.claude-plugin/marketplace.json` that is present MUST be readable: it MUST parse, it MUST carry a `plugins` array, and its entries MUST NOT mix kinds - either every entry's `source` resolves under `skills/` (a catalogue OF SKILLS) or none does (a catalogue OF PLUGINS). The two shapes are read by different tooling paths and a mixed manifest is claimed entirely by the first, so its plugin entries are examined by nothing; an unparseable one is examined by nothing at all. In every case the author declared entries and no tool looks at them (`U17`, `catalogue-manifest-shape`). Introduced at Standard 0.14 and, per sec 7.7, held at `warn` through 0.14; the window closed and it is a gate-failing `error` from 0.15 (ADR 0052, and its 2026-08-18 graduation decision).
- **A catalogue-level finding is not a plugin requirement.** Findings about a catalogue - its manifest shape, its entries, collisions between the members it lists, or a disagreement between its pin and a member's own manifest - are reported against the CATALOGUE and are never numbered requirements laid on an individual plugin. A requirement a plugin cannot discharge by editing its own repository alone, without reference to any other member and without editing the catalogue, is not a requirement (the unilateral-remedy test, ADR 0051). Two of these - collisions on a skill directory or a command name between two members - depend on runtimes sharing one component namespace; if a runtime namespaces components by plugin they cease to be defects at all.
