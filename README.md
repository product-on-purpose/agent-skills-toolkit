...
<p>
  ...
  <img src="https://img.shields.io/badge/version-1.16.2-blue?style=flat-square" alt="Version 1.16.0">
...
- **Bronze - Universal (`U1-U9`, `U11-U17`, 16 checks).** Certifies identical, portable files that run unchanged on any agentskills.io agent. For the beginner on-ramp. Adds a minimal `library.json`, valid skill anatomy, and a description that clears the bar.
...
**`v1.16.2`, Gold grade, installable.** The toolkit is a self-proving example of the Standard it defines: it declares `tier: advanced`, and CI grades it there - the gate is green and `tier-report` prints `advanced` with an empty burndown.
