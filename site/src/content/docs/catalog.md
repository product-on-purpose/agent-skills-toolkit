...
A portable Node spine (one runtime dependency, a YAML parser) grades a plugin against the Standard: Bronze checks `U1`-`U9`, `U11`-`U14`, Silver `S1`-`S8`, and Gold `G1`-`G10` (a 32-check spine). The aggregate gate (`node scripts/check.mjs`) reports the tier and the burndown, runs in CI, and needs no model.
