...
### Added
- Judgment subagent roster (Phase 4, Claude-only per Standard sec 3.3): five reusable bounded delegate roles. `askit-explorer` (broad read-only repo survey, `Read`/`Grep`/`Glob`) and `askit-file-search` (pinpoint search, same tools, distinct breadth) are the discovery pair; `askit-file-ops` (`Read`/`Write`/`Edit`, no `Bash`) is the bounded mutation primitive; `askit-reviewer` (`Read`/`Bash`) is the qualitative review layer and `askit-quality-grader` (`Read`/`Bash`, runs `tier-report`) the tier-grading layer, together with the existing `askit-evaluator` (mechanical) forming a graded-assessment trio. Each declares the narrowest tool set with a documented why (sec 9) and no `chain:` (leaf roles, so no S4 contract entries). Registered in `library.json` components.subagents + INDEX.md.
- `askit-capability-advisor` skill (Phase 4): reports which component types a target agent can run and recommends a conformance tier before building (advise/check modes, area X / Standard sec 2.3, 3.1-3.10). Encodes the Claude/Codex capability matrix (pinned to Codex CLI v0.135): skills/MCP/AGENTS.md portable on both; commands realize on Codex as the backing skill; subagents/output-styles/statuslines Claude-only for plugin distribution; hooks a Codex subset. `check` mode flags a Claude-only component shipped under a `codex` target. Folds in the catalog's check-agent-support.
